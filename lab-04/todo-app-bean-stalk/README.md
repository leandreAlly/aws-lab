...
- **CloudFormation owning `VersionLabel` silently reverted every CI deploy.** I predicted this would bite "the first time the template is edited after CI is live" and underestimated it: GitSync re-syncs the stack on *every push to the repo*, so CloudFormation reasserted `VersionLabel: !Ref InitialApplicationVersion` minutes after each deploy. The workflow went green, the environment moved to the new version, then rolled back to the hand-uploaded bundle. The two deploy paths were not independent — which is the entire premise of splitting them. Fixed with a `ManageVersionLabel` parameter: `true` for the first environment create (it has to launch on the S3 bundle to satisfy the brief), `false` from then on, where `AWS::NoValue` removes the property so CloudFormation stops having an opinion. **The stack was in perfect sync with its template and serving the wrong code because of that, not in spite of it.**
