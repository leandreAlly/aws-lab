...
| `AWS_REGION` | variable | the lab region, e.g. `eu-north-1` |
| `EB_APPLICATION` | variable | `ApplicationNameOutput` |
| `EB_ENVIRONMENT` | variable | `EnvironmentNameOutput` |

Only the two account-identifying values are secrets — that keeps the account ID out of a public repo's logs. A region and a resource name aren't sensitive, and making them secrets would mask them in logs for no benefit.

**None of the five has a default.** The workflow's first step fails with the exact list of what's missing rather than falling back to a guess. An earlier draft defaulted the region, which meant a missing variable would have deployed silently into the wrong region and surfaced as a permissions error rather than a routing one — see *what broke*.
...
| Workflow security | OIDC, no stored keys, branch-pinned trust; permissions scoped where the service allows (see below) |
...
- **Least privilege on a Beanstalk deploy role has a floor, and it is high.** The CI role was built scoped: `s3:PutObject` on `bundles/*`, three Elastic Beanstalk mutations, read-only describes. Deploying then failed four times in a row, each on a different missing action — `s3:CreateBucket` on EB's own working bucket, `cloudformation:GetTemplate` on the `awseb-*` stack EB manages, `autoscaling:SuspendProcesses` on the `awseb-*` ASG, and one more after that. No denial names the next, so each cost a push, a stack update and a deploy to discover. I stopped and attached `AdministratorAccess-AWSElasticBeanstalk`, AWS's own maintained answer. **That is a retreat, and it is recorded as one:** the role can now terminate environments, not merely deploy to them. The scoped inline policies are kept because they still pin access to this lab's bundle bucket, which the managed policy doesn't cover. The real lesson is that "deploy a version to an environment" is not a small permission — underneath, it drives a CloudFormation stack, an Auto Scaling group and two S3 buckets that Beanstalk owns, so a deploy-only role is close to an administer-Beanstalk role however you write it.
...
- **A default for an environment-identifying value hides the failure it causes.** The workflow originally read `${{ vars.AWS_REGION || 'eu-west-1' }}`. If the variable were unset, deploys would have gone to Ireland — where the stack doesn't exist — and failed as *access denied* or *environment not found*, neither of which points at the region. Fallbacks are right for tuning values and wrong for values that identify *which account, region or resource you are acting on*. All three are now required, checked in one preflight step that names everything missing at once.
